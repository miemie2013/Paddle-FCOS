...
class Decode(object):
    ...
    def detect_image(self, image, fetch_list, draw_image, draw_thresh=0.0):
        ...
        boxes, scores, classes = self.predict(pimage, im_info, fetch_list)
    ...
    def detect_batch(self, batch_img, fetch_list, draw_image, draw_thresh=0.0):
        ...
        if pred[0][0] < 0.0:
            ...
            # 1.限制预测框在图片内
            ori_h, ori_w, _ = batch_img[0].shape
            boxes[:, 0] = np.maximum(0, boxes[:, 0])
            boxes[:, 1] = np.maximum(0, boxes[:, 1])
            boxes[:, 2] = np.minimum(ori_w - 1, boxes[:, 2])
            boxes[:, 3] = np.minimum(ori_h - 1, boxes[:, 3])
            # 2.去掉边长是0的预测框
            threshold = 0.0
    ...
    def predict(self, image, im_info, fetch_list):
